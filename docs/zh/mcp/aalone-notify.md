...
description: "💬 Notify MCP Server 提供消息推送的 MCP (Model Context Protocol) 服务器，支持企业微信、钉钉、Telegram、Bark、Lark、飞书、Home Assistant 安装 方式1: uvx yaml { \"mcpServers\": { \"mcp-notify\": { \"command\": \"uvx\", \"args\": [\"mcp-notify\"]…"
---

# MCP通知服务器 - 微信/Telegram

💬 Notify MCP Server 提供消息推送的 MCP (Model Context Protocol) 服务器，支持企业微信、钉钉、Telegram、Bark、Lark、飞书、Home Assistant 安装 方式1: uvx yaml { "mcpServers": { "mcp-notify": { "command": "uvx", "args": ["mcp-notify"]…
