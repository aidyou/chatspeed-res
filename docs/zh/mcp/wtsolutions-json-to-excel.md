...
description: "JSON转Excel MCP（模型上下文协议）提供了一个标准化的接口，用于将JSON数据转换为CSV格式字符串。它包括两个工具：jsontoexcelmcpfromdata和jsontoexcelmcpfromurl，可以将JSON数据或来自URL的JSON文件转换为CSV格式。"
---

# JSON转Excel CSV by WTSolutions

JSON转Excel MCP（模型上下文协议）提供了一个标准化的接口，用于将JSON数据转换为CSV格式字符串。它包括两个工具：jsontoexcelmcpfromdata和jsontoexcelmcpfromurl，可以将JSON数据或来自URL的JSON文件转换为CSV格式。
...
* JSON to Excel MCP Service: Convert JSON to Excel by AI Model MCP SSE/StreamableHTTP request. ( Note:
...
By using JSON to Excel MCP, you agree to the [service agreement](https://github.com/he-yang/json-to-excel-mcp/blob/HEAD/TERMS.md), and [privacy policy](https://github.com/he-yang/json-to-excel-mcp/blob/HEAD/PRIVACY.md).
