...
**And much more.** See the full [tool reference](https://github.com/jfrog/jfrog-mcp-server/blob/HEAD/TOOLS.md) for everything the server exposes.

> **Note:** The JFrog MCP Server is now **generally available (GA)** on JFrog Cloud (SaaS). The self-managed server is in Beta. Individual tools marked **Beta** in the [tool reference](https://github.com/jfrog/jfrog-mcp-server/blob/HEAD/TOOLS.md) are experimental and must be enabled for your environment.
...
> **Note:** Because this is a remote server, the tool set evolves on the server. The [`TOOLS.md`](https://github.com/jfrog/jfrog-mcp-server/blob/HEAD/TOOLS.md) reference reflects the current tool surface.
...
In the examples below, replace `` with your JFrog platform URL (for example, `https://mycompany.jfrog.io`).
...
See the full reference in [`TOOLS.md`](https://github.com/jfrog/jfrog-mcp-server/blob/HEAD/TOOLS.md).
