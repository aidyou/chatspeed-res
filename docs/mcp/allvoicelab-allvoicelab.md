---
title: "趣丸千音"
description: "官方 AllVoiceLab 模型上下文协议(MCP)服务器，支持与强大的文本转语音和视频翻译API交互。允许MCP客户端如Claude Desktop、Cursor、Windsurf、OpenAI Agents等生成语音、翻译视频、智能变声等功能。"
---
...
1. 从中获取API键[AllVoiceLab](https://www.allvoicelab.com/).
2. 安装`uv`（Python软件包管理器），安装`curl -LsSf https://astral.sh/uv/install.sh | sh`
3. **重要**：不同区域中API的服务器地址需要匹配相应区域的键，否则将出现该工具不可用的错误。

|区域|全局|大陆|
|:--|:-----|:-----|
| allvoicelab_api_key |去[AllVoiceLab](https://www.allvoicelab.com/workbench/api-keys)|去[AllVoiceLab](https://www.allvoicelab.cn/workbench/api-keys) |
...
选择一个长文字（例如，“愚蠢的老人去除山脉”），然后问：将此文字翻译成日语。
如果未指定语言，默认情况下它将翻译为英语。
...
**官方网站：** [https://github.com/allvoicelab/AllVoiceLab-MCP](https://github.com/allvoicelab/AllVoiceLab-MCP)
...
资源文件：`resources/mcp/allvoicelab-allvoicelab.json`。内容最后核验于 `2026-08-30`；免费额度和服务限制可能随官方政策变化。
