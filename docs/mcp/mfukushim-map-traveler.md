...
English / [Japanese](https://github.com/mfukushim/map-traveler-mcp/blob/HEAD/README_jp.md)
...
```bash
  python3 -m venv venv
  . venv/bin/activate or .venvScriptsactivate
...
```
4. rembg cli will be installed in the python exe location, so get the path. The file location varies depending on the OS and python installation status, but in the case of venv, it is (virtual environment name)Scriptsembg.exe or (virtual environment name)/bin/rembg above the directory you set. If you can't find it, search for the path with a file search software. Set that path to rembg_path of env in claude_desktop_config.json. (Example: "rembg_path": "C:\Users\xxxx\Documents\rembg_venv\venv\Scripts\rembg.exe")
...
**Official site: ** [https://github.com/mfukushim/map-traveler-mcp](https://github.com/mfukushim/map-traveler-mcp)
**Status: ** `active`　**Last verified: ** `2026-08-30`

## Categories & Tags

- Categories: `data`
- Tags: `location services`, `image and video processing`, `social media`, `chinese`
...
Resource file: `resources/mcp/mfukushim-map-traveler.json`. Content last verified on `2026-08-30`; free quotas and service limits may change with official policies.
